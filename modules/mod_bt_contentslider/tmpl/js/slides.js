		...
		return this.each(function(){
			...
			$('.' + option.container, $(this)).children().wrapAll('<div class="slides_control"/>');
                        if( window.location.hash.indexOf("#") == 0 )
                            option.start = window.location.hash.substr( 1 );
			var elem = $(this),
				...
				if (!active && loaded) {
					...
					switch(direction) {
						...
						case 'pagination':
							...
							prev = $('.' + option.paginationClass + ' li.'+ option.currentClass +' a').attr('href').match('[^#/]+$');
					...
					if (option.pagination) {
						...
						$('.'+ option.paginationClass +' li.' + option.currentClass).removeClass(option.currentClass);
						// add current class to next
						$('.' + option.paginationClass + ' li:eq('+ next +')').addClass(option.currentClass);
					}
			...
			if (option.preload && control.find('img:eq(' + start + ')').length) {
				...
				if ($('img').parent().attr('class') != 'slides_control') {
			...
			if (option.generateNextPrev) {
				$('.' + option.container).after('<a href="#" class="'+ option.prev +'">Prev</a>');
				$('.' + option.prev).after('<a href="#" class="'+ option.next +'">Next</a>');
			}
			...
			if (option.generatePagination) {
				...
				control.children().each(function(){
					$('.' + option.paginationClass).append('<li><a href="#'+ number +'">'+ (number+1) +'</a></li>');
					number++;
				});
			} else {
			...
			$('.' + option.paginationClass + ' li:eq('+ start +')').addClass(option.currentClass);
			
			// click handling 
			$('.' + option.paginationClass + ' li a').click(function(){
				// pause slideshow
				if (option.play) {
					 pause();
				}
